...
"""
Step 3. Since this is a calendar program, we'll need to access date and time quite often. We've been using the familiar datetime function, but sometimes it's necessary to use other functions. Let's try a new function out.
...
"""
Step 6. What data structure can we use to store calendar events? You learned about lists and dictionaries in the Python course, so we'll have to choose one of those two.
...
"""
Step 9. On the next line, inside of the function, print a welcome message to the user. Use concatenation to include the message and the user's first name.
"""
...
"""
Step 11. It's time to use the new strftime function we imported. The cool thing about the strftime function is that printing different units of time (months, days, years, hours, etc.) is simpler than having to call separate functions for each aspect (the way datetime works).
